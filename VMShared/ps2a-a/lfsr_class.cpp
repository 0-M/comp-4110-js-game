...
LFSR::LFSR(const char* seed, const int t) : tap_(t) {  // LFSR constructor
...
int LFSR::step() {  // simulate one step
    int new_bit = registers_[registers_.size() - tap_ - 1] ^ registers_[0];
    ///<  new_bit xor of tap bit and shifted bit
    registers_.erase(registers_.begin());
    registers_.push_back(new_bit);
    return newBit;
}
