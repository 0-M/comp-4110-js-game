...
class Body: public SpaceObject {
 public:
  ...
  Body(double x_velocity, double y_velocity, double ux_velocity,
       double uy_velocity, double mass, double x, double y, double ux,
       double uy, std::string gif_name);

  /**
   * Copy constructor
   * @param const Body &bSource
   * @return N/A
   */
  ...
  //! \brief returns the file name of the gif
  //! \return file name of gif
  //!
  //! returns the file name of the gif
  std::string getGifName();
  /**
   * inherited from drawable and will be inherited from body and star
...
class Universe : public sf::Drawable {
 public:
  ...
  /**
   * \brief returns the vector of bodies
   * \return std::vector<Body>
   * *Unit test purposes only*
   */
  std::vector<Body> getPlanets() const;
